    ...
    <div className="bg-surface-container-lowest/85 backdrop-blur-2xl rounded-[2.5rem] p-6 shadow-[0_24px_48px_rgba(0,0,0,0.08)] border border-white/20">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-black text-primary">₹{(earnings.totalFare * 0.8).toFixed(2)}</p>
        </div>
      </div>

      {/* Wallet Display Section */}
      <div className="flex items-center justify-between mb-8 bg-surface-container-low p-4 rounded-3xl border border-white/40 shadow-sm">
          <div className="text-left">
            ...
            <p className={`text-2xl font-black ${isNegative ? 'text-red-500' : 'text-green-600'}`}>
          ...
          <div>
            {isNegative ? (
               <button 
                 ...
                 className="bg-red-500 text-white px-5 py-2.5 rounded-2xl text-sm font-bold shadow-md shadow-red-500/20 active:scale-95 transition-all">
                 Settle Now
               </button>
            ) : (
               <button 
                  ...
                  className={`px-5 py-2.5 rounded-2xl text-sm font-bold shadow-md transition-all active:scale-95 ${walletData.wallet >= threshold ? 'bg-primary text-on-primary shadow-primary/20 hover:opacity-90' : 'bg-gray-200 text-gray-500 cursor-not-allowed shadow-none'}`}
               >Cashout</button>
            )}
          </div>
      </div>
