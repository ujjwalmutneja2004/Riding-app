...
const CaptainRideHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/captains/history`, {
          headers: {
  ...
  return (
    <div className="h-screen bg-gray-50 p-4 overflow-y-auto">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/captain-dashboard')} className="p-2 bg-white rounded-full shadow-md">
      ...
      {loading ? (
        ...
        <div className="text-center mt-20">
          <i className="ri-steering-2-line text-5xl text-gray-300"></i>
          <p className="text-gray-500 mt-2">No completed rides yet.</p>
        </div>
      ) : (
            ...
            <div key={ride._id} className="bg-white p-4 rounded-xl shadow-sm border">
              ...
              <div className="mt-3 pt-3 border-t flex items-center justify-between">
                 <div className="flex items-center gap-3">
                   ...
                   <div>
                     <p className="text-sm font-medium">{ride.user?.fullname?.firstname || 'Passenger'}</p>
                     <p className="text-xs text-gray-500">{ride.rideMode}</p>
                   </div>
                 </div>
                 {ride.rating && (
                   <div className="flex items-center text-yellow-500 gap-1 font-bold">
                     {ride.rating} <i className="ri-star-fill text-sm"></i>
                   </div>
                 )}
              </div>
            </div>
